...
def main():
    ...
    # print(initial_state)
    SAM_MODEL = Sam(initial_state=initial_state, alphas=alphas, slam_type=filter_name, data_association = DATA_ASSOCIATION, update_type=UPDATE_TYPE, Q=Q)
    ...
    with movie_writer.saving(fig, args.movie_file, data.num_steps) if should_write_movie else get_dummy_context_mgr():
        for t in range(data.num_steps):
            ...
            for i in SAM_MODEL.LEHRBUCH.keys():
                Coord = SAM_MODEL.graph.get_estimated_state()[SAM_MODEL.LEHRBUCH[i]]
            ...
            plt.plot(states_results_x[-1], states_results_y[-1],'bo',markersize=3.0)              
    ...
    plot2dcov(np.array([states_results_x[-1],states_results_y[-1]]).T,COV.A,'k',nSigma=3)
    plt.show(block=True)
    ...
    plt.spy(SAM_MODEL.graph.get_adjacency_matrix(), marker = 'o', markersize = 2.0, color = 'g')
    plt.title('GAM')
    plt.show(block=True)

    plt.figure(figsize = (8,8))
    plt.spy(SAM_MODEL.graph.get_information_matrix(), marker = 'o', markersize = 2.0, color = 'g')
    plt.title('GIM')
    plt.show(block=True)
